...
export class DisponibilidadComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.productoService.getProductoPorId(this.id_producto).subscribe(
      ...
      (err) => {
        console.log(err);
        this.toastr.error(
          'Hubo un problema al buscar el detalle del producto',
          'Ups!',
          {
            timeOut: 0,
          }
  ...
  agregar(agregar: HTMLSelectElement, cantidad: HTMLInputElement) {
    ...
    this.productoService
      .agregarUnidadProductoId(this.id_producto, agregar.value, cantidad.value)
      .subscribe(
        (res) => {
          this.producto = res;
          this.toastr.clear();
  ...
  retirar(eliminar: HTMLSelectElement, cantidad: HTMLInputElement) {
    ...
    this.productoService
      .botarCantidadProductoId(this.id_producto, eliminar.value, cantidad.value)
      .subscribe(
        (res) => {
          this.producto = res;
          this.toastr.clear();
          console.log(res);
          this.toastr.success(
            'las unidades se eliminaron correctamente',
            '¡Listo!',
            {
              timeOut: 0,
            }
